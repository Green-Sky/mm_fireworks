...
void OrganizerSceneService::sceneFixedUpdate(Engine&) {
	...
	while (_accumulator >= static_cast<decltype(_accumulator)>(dt)){
		...
		auto& frame_time = _scene->ctx<MM::Components::OrgFrameTime>();
		frame_time.fixedDelta = delta_factor * f_delta;
...
void OrganizerSceneService::updateOrganizerVertices(Scene& scene) {
	...
	if (!scene.try_ctx<MM::Components::OrgFrameTime>()) {
		scene.set<MM::Components::OrgFrameTime>();
	}
